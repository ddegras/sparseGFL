...
//   Rcout << "Iteration 0 Objective " << objective << std::endl;  
   vec beta_best = beta;
...
// FIND SUBGRADIENT OF MINIMUM NORM (MIN_u,v || z - U - V * Dt ||_F)
...
// mat arg = z - u - v * Dt;
double objective = arma::norm(z-u-v*Dt,"fro");
if (objective <= tol) return (z-u-v*Dt); 
...
for (i=1; i<=maxit; i++) {
   ...
   // Gradient step
   grad_u = u_adj + v_adj * Dt - z;
//   grad_v = grad_u * D; 
//   grad_v = diff(grad_u,1,1);
   u = u_adj - 0.2 * grad_u;
...
//   if (i % 9 == 0) 
//      objective = arma::norm(z-u-v*Dt,"fro");
   if (i % 10 == 0) {
      objective_old = objective;
      objective = arma::norm(z-u-v*Dt,"fro");
      if (objective < objective_best) {
         objective_best = objective;
         v_best = v;}
      if (objective_best <= tol ||
         abs(objective-objective_old) <= tol * objective_old)
            break;  
   }
} // end for

u = clamp(z-v_best*Dt,nlam1,lam1);
u(fixg).zeros();
return (z-u-v_best*Dt);
}
