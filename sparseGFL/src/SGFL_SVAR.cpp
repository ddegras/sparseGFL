   ...
   const mat & Anext, const bool lag0, const bool intercept, const double tol)
{
   ...
   if (!is_null_Aprev) {
      ...
      if (single_candidate) {
         if (norm(grad,"fro") <= (lam2*(wprev+wnext))) 
            {return Aprev;} else {return null_mat;}   
      } else if (norm(grad,"fro") <= lam2*wprev) {return Aprev;}
   }
...
//               A = soft_svar(Z,lam1new/L,intercept);
               break;
            case 1:
               if (first) {
   ...
   const uword maxit, const double tol_fista, const double tol_equal)
{
   ...
   double val;
   for (g=0; g<nchain; g++) {
      d = svd(x.cols(startc(g),endc(g)));
      val = (d(0) * d(0)) + (lam3 * len(g));
