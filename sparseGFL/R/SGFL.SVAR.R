   ...
   parallel=FALSE, verbose=FALSE)
{
   ...
   ## Numerical controls
   con <- list(maxit.bd = 1000, tol.bd = 1e-7, maxit.fista = 1000,
      tol.fista = 1e-8, maxit.fp = 1e4, tol.fp = 1e-7, 
      tol.cp = 1e-6, maxit.pg = 100, tol.pg = 1e-5, 
      tol.equal = 1e-8)
   ...
   ## Check if package RSpectra is available for quickly 
   ## finding largest eigenvalue of matrix
   ...
   progressBlock <- TRUE # @@@@@@@@@@
   progressFusion <- TRUE # @@@@@@@@@@
   chain.counter <- 0 # counter for convergence of fusion chains
   chain.counter.c <- 5 # critical value for chain convergence 
   change <- NULL   
   ...
   repeat
   {
      ...
      for (j in 1:pattern[1])
      {
         ...
         for (i in sweep.index) 
         {
            ...
            if (progress && !identical(A[,,i],A.tmp)) {   
               A[,,i] <- A.tmp
      ...
      for (j in 1:pattern[2])
      {
         ...
         # if (pattern[2] < 1 || count > con$maxit.bd)  # || !progressFusion) 
         	# { progressFusion <- FALSE ; break } 
         if (pattern[2] < 1 || count > con$maxit.bd)  # || !progressFusion) 
         	{ progressFusion <- FALSE ; break } 
         ...
         for (i in sweep.index) 
         {
            ...
            L.index <- ifelse(test, svds(x[,chain],1,0,0)$d^2, 
               svd(x,0,0)$d[1]^2) 
            ...
            ## If not, FISTA
               result <- FISTA_1_svar(A[,,i], x[,chain,drop=FALSE], 
                  ...
                  wnext, Anext, L.index, lag0, intercept, maxit.fista,
      ...
      if ((!progress || convergence) && !single.chain && count <= maxit.bd) {
         ...
         startc <- c(1L,change) 
         endc <- c(change-1L,n) 
         result <- FISTA_M_svar(A[,,startc,drop=FALSE], x, y, lam1, lam2,  
            lam3, w, lag0, intercept, startc-1L, endc-1L, eta=1.25, 
            maxit.fista, tol.fista, tol.equal)   
      ...
      if ((!progress) || convergence || count > maxit.bd) {
         ...
         if (verbose) 
            cat("Subgradient method (variable chains) ",count-1," ",sep="")      
         objective[count] <- objective[count-1]
         ...
         ## Line search
         if (j > 1) {
            opt <- optimize(LineSearch, tol=grid[j-1],
               interval=c(grid[j-1],grid[j]))
   ...
   ## Accept simplified solution if does not increase objective too much 
   ##@@ Comment out for performance analysis @@##
   dim(A.tmp) <- dimA
   objective.tmp <- 
      objective_global_svar(A.tmp,x,y,lam1,lam2,lam3,w,intercept)
   if (objective.tmp <= (1+tol.bd)*objective[count]) {
      A <- A.tmp
      objective[count] <- objective.tmp
   } else {
   ...
   return(list(A=A, objective=objective[count], changePoints=change+p,
      trace=objective, subgradient=g, iters=count-1, lambda1=lambda1, 
      lambda2=lambda2, alpha=alpha, time=t2-t1, time2=c(t1,timing,t2)))
}   
