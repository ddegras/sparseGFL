   ...
   control=list(), parallel=FALSE, verbose=FALSE)
{
   ...
   ## Numerical controls
   con <- list(maxit.bd = 1000, tol.bd = 1e-6, 
      maxit.fista = 100, tol.fista = 1e-6, 
      maxit.fp = 1e4, tol.fp = 1e-7, tol.cp = 1e-3,    
   ...
   if (is.null(L)) {
	# svdfun <- if (RSpectra.flag && N >= 3) {
   	...
   	# } else { function(x) (svd(x)$d[1])^2 }	
		...
		L <- (apply(x,3,norm,type="2"))^2 + lam3
   }
   L.fusion <- matrix(,0,3)
   colnames(L.fusion) <- c("start","end","L")
      ...
      for (j in 1:pattern[2])
      {
        ...
        if (verbose) cat("Fusion cycle ",count-1," ", sep="")
         ...
         # not from 1 to n 
            ...
            ## Lipschitz constant
            idx <- which(L.fusion[,"start"] == startc & 
               L.fusion[,"end"] == endc)
            if (length(idx)) {
            	   L.chain <- L.fusion[idx,"L"]
            } else {
               xchain <- aperm(x[,,chain,drop=FALSE],
                  perm=c(1,3,2))
               dim(xchain) <- c(N*flen,p)
               # test <- (RSpectra.flag && N*flen >= 3)   
               # L.chain <- ifelse(test, svds(xchain,1,0,0)$d^2, 
                  # svd(xchain,0,0)$d[1]^2) + (lam3 * flen)
               L.chain <- norm(xchain,"2")^2 + (lam3 * flen)
               L.fusion <- rbind(L.fusion,
                  t(c(startc,endc,L.chain)))
               rm(xchain)
            }
            ...
            ## If not, FISTA
               result <- FISTA_1(
                  ...
                  y[,chain,drop=FALSE], lam1, lam2, lam3,
                  wprev, bprev, wnext, bnext, L.chain,intercept,
